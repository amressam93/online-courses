...
@section('content')
    @include('admin.users.partials.header', ['title' => __('Update Quiz')])
                        ...
                        <form method="post" action="{{ route('quizzes.update',$quiz) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Quiz Name') }}" value="{{ old('name',$quiz->name) }}" required autofocus>
                                ...
                                <div class="text-center">
                                    <button type="submit" name="updateQuiz" class="btn btn-success mt-4">{{ __('Update') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
