                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('quizzes.show',$quiz->id) }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('quiz.question.store',$quiz->id) }}" autocomplete="off">
                            @csrf
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('answers') ? ' has-danger' : '' }}">
                                    ...
                                    <textarea  name="answers" id="input-answers" class="form-control form-control-alternative{{ $errors->has('answers') ? ' is-invalid' : '' }}" placeholder="{{ __('Answers') }}" required autofocus>{{ old('answers') }}</textarea>
                                ...
                                <div class="form-group{{ $errors->has('right_answer') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="right_answer" id="input-right_answer" class="form-control form-control-alternative{{ $errors->has('right_answer') ? ' is-invalid' : '' }}" placeholder="{{ __('Right_answer') }}" value="{{ old('right_answer') }}" required >
